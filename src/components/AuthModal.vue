<script setup>
	...
	function register() {
		...
		const { name, email, password, currency } = form.value;

		axios
			.post(
				`${env.VITE_BE_API}/auth/register`,
			...
			.catch((error) => {
				alert.error(
					error.message.includes("timeout")
						? t("authModal.errors.networkError")
						: t("authModal.errors.invalidCredentials")
				);
			})
			.finally(() => {
								...
								<div class="fv-row mb-10">
									<div class="d-flex flex-stack mb-2">
										...
										>
											{{
												$t(
													"authModal.login.forgotPassword"
												)
											}}
										</a>
									</div>
								...
								<div class="text-center">
									...
									>
										<span v-if="!loadingReg">{{
											$t("authModal.login.submitText")
										}}</span>
										<span v-else>
											{{
												$t(
													"authModal.login.loadingText"
												)
											}}
											<span
							...
							>
								<div class="mb-10 text-center">
									...
									<div class="text-gray-400 fw-semibold fs-4">
										{{
											$t(
												"authModal.register.alreadyHaveAccount"
											)
										}}
										<a
											@click="signIn = 'login'"
								...
								<div class="fv-row mb-7">
									<label
										class="form-label fw-bold text-dark fs-6"
										>{{
											$t("authModal.register.emailLabel")
										}}</label
									>
									<input
										class="form-control form-control-lg form-control-solid"
								...
								<div class="fv-row mb-10">
									<label
										...
										>{{
											$t(
												"authModal.register.passwordLabel"
											)
										}}</label
									>
									<input
								...
								<div class="fv-row mb-5">
									<label
										...
										>{{
											$t(
												"authModal.register.confirmPasswordLabel"
											)
										}}</label
									>
									<input
								...
								<div class="text-center">
									...
									>
										<span v-if="!loadingReg">{{
											$t("authModal.register.submitText")
										}}</span>
										<span v-else>
											{{
												$t(
													"authModal.register.loadingText"
												)
											}}
											<span
												class="spinner-border spinner-border-sm align-middle ms-2"
